...
public class BookingController {
	...
	public ResponseData<List<BookingResponse>> getAllBooking() {
		return ResponseData.<List<BookingResponse>>builder().message(ResponseMessage.GET_ALL_BOOKING)
				.data(bookingService.getAllBooking()).build();
	}

	@DeleteMapping("/cancel-booking/{bookingId}")
	public ResponseData<BookingCancelResponse> cancelBooking(@PathVariable Integer bookingId) {
		return ResponseData.<BookingCancelResponse>builder().message(ResponseMessage.CANCEL_BOOKING)
	...
	public ResponseData<BookingResponse> createBookingForChoreographer(@RequestBody BookingRequest bookingRequest) {
		return ResponseData.<BookingResponse>builder().message(ResponseMessage.CREATE_BOOKING)
				.data(bookingService.createBookingRequestForChoreography(bookingRequest)).build();
	}
	...
	public ResponseData<BookingResponse> createBookingForDancers(@RequestBody BookingRequest bookingRequest) {
		return ResponseData.<BookingResponse>builder().message(ResponseMessage.CREATE_BOOKING)
				.data(bookingService.createBookingRequestForDancer(bookingRequest)).build();
	}

	@PutMapping("/accept/{bookingId}")
	public ResponseData<BookingResponse> acceptBookingRequest(@PathVariable Integer bookingId) {
		return ResponseData.<BookingResponse>builder().message(ResponseMessage.ACCEPT_BOOKING_SUCESSFULLY)
				.data(bookingService.acceptBookingRequest(bookingId)).build();
	}
	...
	public ResponseData<BookingResponse> startWorkRequest(@PathVariable Integer bookingId) {
		return ResponseData.<BookingResponse>builder().message(ResponseMessage.START_WORK_SUCCESSFULLY)
				.data(bookingService.startWork(bookingId)).build();
	}

	@PutMapping("/end-work-request")
	public ResponseData<BookingResponse> endWorkRequest(@RequestBody EndWorkRequest bookingId) {
		return ResponseData.<BookingResponse>builder().message(ResponseMessage.END_WORK_SUCCESSFULLY)
				.data(bookingService.endWorking(bookingId)).build();
	}
