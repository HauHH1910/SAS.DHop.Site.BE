...
public class UserController {
	...
	public ResponseData<List<UserResponse>> getAllUser() {
		return ResponseData.<List<UserResponse>>builder().message(ResponseMessage.GET_ALL_USER)
				.data(userService.getAllUser()).build();
	}

	@GetMapping("/info")
	public ResponseData<UserResponse> getUserInfo() {
		return ResponseData.<UserResponse>builder().message(ResponseMessage.GET_USER_INFO)
				.data(userService.getUserInfo()).build();
	}

	@PostMapping
	public ResponseData<UserResponse> createUser(@RequestBody CreateUserRequest request) {
		return ResponseData.<UserResponse>builder().message(ResponseMessage.CREATE_USER)
	...
	@PatchMapping("/{id}")
	public ResponseData<UserResponse> updateUser(@PathVariable("id") Integer id,
			@RequestBody UpdateUserRequest request) {
		return ResponseData.<UserResponse>builder().message(ResponseMessage.UPDATE_USER)
				.data(userService.updateUser(id, request)).build();
	}

	@GetMapping("/{id}")
	public ResponseData<UserResponse> getUser(@PathVariable("id") Integer id) {
		return ResponseData.<UserResponse>builder().message(ResponseMessage.GET_USER).data(userService.getUser(id))
				.build();
	}

	@GetMapping("/search")
	public ResponseData<UserResponse> findByEmail(@RequestParam String email) {
		return ResponseData.<UserResponse>builder().message(ResponseMessage.FIND_EMAIL)
				.data(userService.findUser(email)).build();
	}
}
