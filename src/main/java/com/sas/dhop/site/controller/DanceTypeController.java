...
public class DanceTypeController {
	...
	public ResponseData<List<DanceTypeResponse>> getAllDanceType() {
		return ResponseData.<List<DanceTypeResponse>>builder().message(ResponseMessage.GET_ALL_DANCE_TYPE)
				.data(danceTypeService.getAllDanceType()).build();
	}

	@PostMapping
	public ResponseData<DanceTypeResponse> createDanceType(@RequestBody DanceTypeRequest request) {
		return ResponseData.<DanceTypeResponse>builder().message(ResponseMessage.CREATE_DANCE_TYPE)
				.data(danceTypeService.createDanceType(request)).build();
	}
}
