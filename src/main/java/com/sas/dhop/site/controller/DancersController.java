...
public class DancersController {
	...
	public ResponseData<List<DancerResponse>> getAllDancer() {
		return ResponseData.<List<DancerResponse>>builder().message(ResponseMessage.GET_ALL_DANCERS)
				.data(dancerService.getallDancer()).build();
	}

	@PatchMapping("/update-dancer-profile/{dancerId}")
	public ResponseData<DancerResponse> updateDancer(@PathVariable Integer dancerId,
			@RequestBody DancerRequest dancerRequest) {
		return ResponseData.<DancerResponse>builder().message(ResponseMessage.UPDATE_DANCER)
				.data(dancerService.updateDancer(dancerId, dancerRequest)).build();
	}

	@DeleteMapping("/remove-dancers/{dancerId}")
	public ResponseData<DancerResponse> removeDancer(@PathVariable Integer dancerId) {
		return ResponseData.<DancerResponse>builder().message(ResponseMessage.REMOVE_DANCER)
				.data(dancerService.removeDancer(dancerId)).build();
	}

	@GetMapping("/get-dancer-by-Id/{dancerId}")
	public ResponseData<DancerResponse> getDancerById(@PathVariable Integer dancerId) {
		return ResponseData.<DancerResponse>builder().message(ResponseMessage.GET_DANCER)
				.data(dancerService.getDancerById(dancerId)).build();
	}
	...
	public ResponseData<DancerResponse> getDancerByDanceType(@PathVariable Integer danceTypeId) {
		return ResponseData.<DancerResponse>builder().message(ResponseMessage.GET_DANCER_BY_DANCE_TYPE)
				.data(dancerService.getDancerByDanceType(danceTypeId)).build();
	}
}
