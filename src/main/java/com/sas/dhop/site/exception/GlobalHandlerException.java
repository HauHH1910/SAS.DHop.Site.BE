...
public class GlobalHandlerException {
	...
	public ResponseEntity<ResponseError> handleBusinessException(BusinessException e) {
		...
		return ResponseEntity.status(errorConstant.getHttpStatusCode()).body(
				ResponseError.builder().code(errorConstant.getCode()).message(errorConstant.getMessage()).build());
	}
	...
	public ResponseEntity<ResponseError> handleException(RuntimeException e) {
		...
		return ResponseEntity.status(ErrorConstant.UNCATEGORIZED_ERROR.getHttpStatusCode())
				.body(ResponseError.builder().code(ErrorConstant.UNAUTHENTICATED.getCode())
						.message(ErrorConstant.UNCATEGORIZED_ERROR.getMessage()).build());
	}
	...
	public ResponseEntity<ResponseError> handleValidation(MethodArgumentNotValidException e) {
		...
		return ResponseEntity.badRequest().body(ResponseError.builder().code(errorConstant.getCode())
				.message(mapAttribute(errorConstant.getMessage(), attributes)).build());
	}
