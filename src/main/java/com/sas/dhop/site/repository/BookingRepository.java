...
public interface BookingRepository extends JpaRepository<Booking, Integer>, JpaSpecificationExecutor<Booking> {
    ...
    // @Query("SELECT COUNT(b) FROM Booking b WHERE b.bookingDate BETWEEN :startDate
    // AND :endDate")
    ...
    // @Query("SELECT COUNT(b) FROM Booking b WHERE YEAR(b.bookingDate) =
    // YEAR(CURRENT_DATE) AND
    // MONTH(b.bookingDate)
    // = MONTH(CURRENT_DATE)")
    ...
    // BigDecimal getTotalAmountOfActivatedBookingsByDancerId(@Param("dancerId")
    // Integer dancerId);
    //
    // @Query("SELECT COALESCE(SUM(b.price), 0) FROM Booking b WHERE
    // b.status.statusName =
    ...
    // BigDecimal
    // getTotalAmountOfActivatedBookingsByChoreographyId(@Param("choreographyId")
    ...
    // Long countByChoreographyIdAndStatusId(Integer choreographyId, Integer
    // statusId);
    // List<Booking> findTop5ByDancerIdAndStatusIdOrderByStartAsc(Integer dancerId,
    // Integer
    // statusId);
    // List<Booking> findTop5ByChoreographyIdAndStatusIdOrderByStartAsc(Integer
    // choreographyId,
    // Integer statusId);
}
