...
public interface BookingRepository extends JpaRepository<Booking, Integer>, JpaSpecificationExecutor<Booking> {
    ...
    //    Long countBookingsBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime
    // endDate);
    //
    //    // Get total bookings in current month
    //    @Query("SELECT COUNT(b) FROM Booking b WHERE YEAR(b.bookingDate) = YEAR(CURRENT_DATE) AND MONTH(b.bookingDate)
    // = MONTH(CURRENT_DATE)")
    ...
    //    @Query("SELECT COUNT(b) FROM Booking b WHERE YEAR(b.bookingDate) = YEAR(CURRENT_DATE) AND WEEK(b.bookingDate)
    // = WEEK(CURRENT_DATE)")
    ...
    //
    //    @Query("SELECT COALESCE(SUM(b.price), 0) FROM Booking b WHERE b.status.statusName = 'BOOKING_ACTIVATE' AND
