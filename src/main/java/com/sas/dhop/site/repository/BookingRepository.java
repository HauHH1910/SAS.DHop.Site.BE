...
//    @Query("SELECT COUNT(b) FROM Booking b WHERE YEAR(b.bookingDate) = YEAR(CURRENT_DATE) AND MONTH(b.bookingDate) = MONTH(CURRENT_DATE)")
...
//    @Query("SELECT COUNT(b) FROM Booking b WHERE YEAR(b.bookingDate) = YEAR(CURRENT_DATE) AND WEEK(b.bookingDate) = WEEK(CURRENT_DATE)")
...
//    @Query("SELECT COALESCE(SUM(b.price), 0) FROM Booking b WHERE b.status.statusName = 'BOOKING_ACTIVATE'")
//    BigDecimal getTotalAmountOfActivatedBookings();
//
//    @Query("SELECT COALESCE(SUM(b.price), 0) FROM Booking b WHERE b.status.statusName = 'BOOKING_ACTIVATE' AND b.dancer.id = :dancerId")
//    BigDecimal getTotalAmountOfActivatedBookingsByDancerId(@Param("dancerId") Integer dancerId);
//
//    @Query("SELECT COALESCE(SUM(b.price), 0) FROM Booking b WHERE b.status.statusName = 'BOOKING_ACTIVATE' AND b.choreography.id = :choreographyId")
//    BigDecimal getTotalAmountOfActivatedBookingsByChoreographyId(@Param("choreographyId") Integer choreographyId);
//
...
//    List<Booking> findTop5ByDancerIdAndStatusIdOrderByStartAsc(Integer dancerId, Integer statusId);
//    List<Booking> findTop5ByChoreographyIdAndStatusIdOrderByStartAsc(Integer choreographyId, Integer statusId);
}
