...
//        Long completedBookings = bookingRepository.countByDancerIdAndStatusId(request.dancerId(), 3); // Assuming 3 is completed status
//        Long pendingBookings = bookingRepository.countByDancerIdAndStatusId(request.dancerId(), 1); // Assuming 1 is pending status
//
//        BigDecimal totalEarnings = bookingRepository.getTotalAmountOfActivatedBookingsByDancerId(request.dancerId());
//
//        List<Booking> upcomingBookings = bookingRepository.findTop5ByDancerIdAndStatusIdOrderByStartAsc(request.dancerId(), 1);
//        List<BookingFeedback> recentFeedbacks = bookingFeedbackService.getFeedbackByDancerId(request.dancerId());
//
...
//        Long completedBookings = bookingRepository.countByChoreographyIdAndStatusId(request.choreographerId(), 3);
//        Long pendingBookings = bookingRepository.countByChoreographyIdAndStatusId(request.choreographerId(), 1);
//
//        BigDecimal totalEarnings = bookingRepository.getTotalAmountOfActivatedBookingsByChoreographyId(request.choreographerId());
//
//        List<Booking> upcomingBookings = bookingRepository.findTop5ByChoreographyIdAndStatusIdOrderByStartAsc(request.choreographerId(), 1);
//        List<BookingFeedback> recentFeedbacks = bookingFeedbackService.getFeedbackByChoreographer(request.choreographerId());
//
//        UserSubscription currentSubscription = userSubscriptionRepository.findByUserId(request.choreographerId())
//            .orElseThrow(() -> new RuntimeException("No subscription found"));
//
//        List<Dancer> topDancers = dancerRepository.findTop5ByOrderByPriceDesc();
//
