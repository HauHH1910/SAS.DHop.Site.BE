...
public class AuthenticationServiceImpl implements AuthenticationService {
	...
	public void register(RegisterRequest request) throws MessagingException {
		...
		User user = User.builder().name(request.name()).phone(request.phone()).email(request.email())
				.password(passwordEncoder.encode(request.password())).roles(roles).status(userStatus).build();
		...
		if (RoleName.CHOREOGRAPHY.equals(request.role()) && request.choreography() != null) {
			...
			Set<DanceType> danceTypes = request.choreography().danceType().stream().map(danceTypeService::findDanceType)
					.collect(Collectors.toSet());

			Choreography choreography = Choreography.builder().danceTypes(danceTypes).user(user)
					.about(request.choreography().about()).yearExperience(request.choreography().yearExperience())
					.status(choreographyStatus).build();
			...
			Set<DanceType> danceTypes = request.dancer().danceType().stream().map(danceTypeService::findDanceType)
					.collect(Collectors.toSet());

			Dancer dancer = Dancer.builder().about(request.dancer().about())
					.dancerNickName(request.dancer().dancerNickName()).user(user).danceTypes(danceTypes)
					.yearExperience(request.dancer().yearExperience()).teamSize(request.dancer().teamSize())
					.status(dancerStatus).build();
	...
	public boolean authenticationChecking(String role) {
		...
		return authentication != null && !(authentication instanceof AnonymousAuthenticationToken)
				&& authentication.isAuthenticated() && authentication.getAuthorities().stream().anyMatch(authority -> {
	...
	private ExchangeTokenResponse getTokenResponse(String code) {
		return identityClient.exchangeToken(ExchangeTokenRequest.builder().code(code).clientId(CLIENT_ID)
				.clientSecret(CLIENT_SECRET).redirectUri(REDIRECT_URI).grantType(GRANT_TYPE).build());
	}

	private User onBoardUserOAuth(OAuthUserResponse userInfo, Status status, Set<Role> roles) {
		return userRepository.findByEmail(userInfo.email())
				.orElseGet(() -> userRepository.save(User.builder().status(status).roles(roles).email(userInfo.email())
						.name(String.join(" ", userInfo.givenName(), userInfo.familyName())).avatar(userInfo.picture())
						.build()));
	}
}
