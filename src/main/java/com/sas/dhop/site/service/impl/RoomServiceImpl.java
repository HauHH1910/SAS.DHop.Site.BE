...
public class RoomServiceImpl implements RoomService {
  ...
  public Room createOrGetRoom(CreateRoomRequest request) {
    return roomRepository
        .findByRoomId(request.roomId())
        .orElseGet(
            () -> {
              log.info(
                  "[Room {} by {} are being created sir]", request.roomId(), request.roomName());
              return roomRepository.save(
                  Room.builder().roomId(request.roomId()).name(request.roomName()).build());
            });
  }
