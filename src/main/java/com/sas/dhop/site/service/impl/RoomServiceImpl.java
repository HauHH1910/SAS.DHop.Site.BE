...
public class RoomServiceImpl implements RoomService {
	...
	public Room createOrGetRoom(CreateRoomRequest request) {
		return roomRepository.findByRoomId(request.roomId()).orElseGet(() -> {
			...
			return roomRepository.save(Room.builder().roomId(request.roomId()).name(request.roomName()).build());
		});
	}
	...
	public List<Message> getMessages(String roomId) {
		return roomRepository.findByRoomId(roomId)
				.orElseThrow(() -> new BusinessException(ErrorConstant.ROOM_NOT_FOUND)).getMessages();
	}
	...
	public RoomDetailResponse joinRoom(String roomId) {
		...
		List<Integer> participantIds = Arrays.stream(split).map(Integer::parseInt).toList();
		...
		Integer otherParticipantId = participantIds.stream()
				.filter(participantId -> !participantId.equals(currentUser.getId())).findFirst()
				.orElseThrow(() -> new BusinessException(ErrorConstant.PARTICIPANT_NOT_FOUND));

		User anotherParticipant = userRepository.findById(otherParticipantId)
				.orElseThrow(() -> new BusinessException(ErrorConstant.USER_NOT_FOUND));

		String lastMessageContent = "Không có tin nhắn";

		if (!room.getMessages().isEmpty()) {
			lastMessageContent = room.getMessages().get(room.getMessages().size() - 1).getContent();
		}

		return new RoomDetailResponse(anotherParticipant.getName(), room.getName(), anotherParticipant.getAvatar(),
				room.getRoomId(), lastMessageContent);
	}
	...
	public RoomResponse getRoom(Principal principal) {
		...
		rooms.forEach(room -> {
			...
			if (participantIds.size() == 2) {
				Integer otherParticipantId = participantIds.stream().filter(id -> !id.equals(user.getId())).findFirst()
						.orElseThrow(() -> new BusinessException(ErrorConstant.PARTICIPANT_NOT_FOUND));
				...
				chatRooms.add(new RoomDetailResponse(otherParticipant.getName(), room.getName(),
						otherParticipant.getAvatar(), room.getRoomId(), lastMessage));
			} else if (participantIds.size() > 2) {
				String lastMessage = (room.getMessages().isEmpty())
						? "Không có tin nhắn"
						: room.getMessages().get(room.getMessages().size() - 1).getContent();

				groupRooms.add(new RoomDetailResponse(room.getName(), room.getName(), "logo-icon.png", room.getRoomId(),
						lastMessage));
			}
		});
