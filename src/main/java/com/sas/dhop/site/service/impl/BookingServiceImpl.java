...
public class BookingServiceImpl implements BookingService {
	...
	public BookingResponse createBookingRequestForDancer(BookingRequest request) {
		...
		Booking booking = Booking.builder().customer(customer).dancer(dancer).area(area).choreography(null)
				.status(status).danceType(Set.of(danceType)).bookingDate(Instant.now()).startTime(request.startTime())
				.endTime(request.endTime()).address(request.address()).detail(request.detail())
				.customerPhone(request.customerPhone()).dancerPhone(request.dancerPhone())
				.numberOfTrainingSessions(request.numberOfTrainingSessions())
				.price(calculateCommissionPrice(request.bookingPrice())).build();
	...
	public BookingResponse createBookingRequestForChoreography(BookingRequest request) {
		Choreography choreography = choreographyRepository.findById(request.choreographyId())
		...
		Booking booking = Booking.builder().customer(customer).dancer(null).area(area).choreography(choreography)
				.status(status).danceType(Set.of(danceType)).bookingDate(Instant.now()).startTime(request.startTime())
				.endTime(request.endTime()).address(request.address()).detail(request.detail())
				.customerPhone(request.customerPhone()).dancerPhone(request.dancerPhone())
	...
	public List<BookingResponse> getAllBooking() {
		return bookingRepository.findAll().stream().map(BookingResponse::mapToBookingResponse).toList();
	}
