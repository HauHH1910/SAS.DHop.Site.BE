    ...
    Area area =
        areaRepository
            ...
            .orElseThrow(
                () -> {
                  log.error(
                      "[Booking for choreography] Area not found with id: {}", request.areaId());
                  return new BusinessException(ErrorConstant.AREA_NOT_FOUND);
                });
  ...
  public BookingResponse endWorking(EndWorkRequest request) {
    ...
    if (request.multipartFiles() != null) {
      List<MediaResponse> mediaResponses =
          cloudStorageService.uploadImage(request.multipartFiles());
    }
  ...
  private BigDecimal calculateCommissionPrice(BigDecimal price) {
    if (price.compareTo(new BigDecimal("200000")) >= 0
      ...
      return price.multiply(new BigDecimal("1.2")).setScale(2, RoundingMode.HALF_UP);
        ...
        && price.compareTo(new BigDecimal("1500000")) < 0) {
      // Phí hoa hồng 15%
